...
class FedMD():
    ...
    def collaborative_training(self):
        ...
        r = 0
        threshold_ratio = 0.25  # initially 25% models are selected
        while True:
            ...
            # Get indices of top 'threshold_ratio' models
            ind = [index for acc, index in acc_and_indices[:int(len(acc_and_indices)*threshold_ratio)]]
            ...
            r+= 1
            # Gradually increase the threshold up to 50%
            threshold_ratio = min(0.50, threshold_ratio + 0.02)
            if r > self.N_rounds:
                break
